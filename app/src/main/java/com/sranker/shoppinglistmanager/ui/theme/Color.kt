package com.sranker.shoppinglistmanager.ui.theme

import androidx.compose.ui.graphics.Color

// Unified Deep Background
val MinimalistBackground = Color(0xFF151515)

// Theme Accents (Primary)
val OceanTeal = Color(0xFF14B8A6)
val ForestGreen = Color(0xFF4ADE80)
val SunsetPink = Color(0xFFD34273) // Pink accent from reference
val SnowSlate = Color(0xFF94A3B8)

// Theme Text/Icon Colors (OnBackground/OnSurface)
val OceanText = Color(0xFFB9F1E8)
val ForestText = Color(0xFFD2F8D0)
val SunsetText = Color(0xFFE5DEAE) // Pale yellow from reference
val SnowText = Color(0xFFFFFFFF)
